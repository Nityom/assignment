...
export default function HeroSection({ className = '' }: HeroSectionProps) {
  ...
  const gradientBackground = 'linear-gradient(180deg, #F5F5F5 40%, #4787FF 400%)';
      ...
      >
        <div className="px-6 md:px-20 pt-16 md:pt-20 pb-10 text-center">
          ...
          <h1 className="font-bold mb-6">
            <div className={`${headlineBaseStyle} text-[#316BFF] tracking-tight`}>
              Your Partner in Digital
            </div>
            
            <div className="flex justify-center my-3">
              <div className={`h-1 w-[80%] bg-gradient-to-r from-[${mainBlueColor}33] via-[${mainBlueColor}] to-[${mainBlueColor}33] rounded-md`} />
            </div>
            
            <div className={`bg-gradient-to-b from-[${mainBlueColor}] via-[#86AEFF80] to-[#DEEBFF4D] text-transparent bg-clip-text ${headlineBaseStyle}`}>
              Innovation
            </div>
          </h1>
          ...
          {/* CTA */}
          <button 
            className={`bg-white text-[${mainBlueColor}] px-6 md:px-8 py-3 rounded-3xl text-base md:text-lg font-medium transition-all duration-200 shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 border border-gray-200`}
            aria-label="Explore Services"
          >
            Explore Services
          </button>

          {/* Go to Events toggle - Simplified responsive classes */}
